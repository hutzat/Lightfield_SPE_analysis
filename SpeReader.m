...
classdef SpeReader < hgsetget
    ...
    methods(Access='public')
        ...
        % Operations
        %------------------------------------------------------------------
        function [frameTimes,frames] = read(obj, varargin)
            ...
            [frameTimes,frames] = parseFrames(obj, frameset);
        end
    end
    ...
    methods (Access='private', Hidden)
        ...
        function [frameTimes,frames] = parseFrames(obj, frameset)
...
%                 fseek(fd, seekstart, 'bof');
                ...
                frames = zeros([height,numFrames],dataType);
                frameTimes=zeros([2,numFrames],'double'); %extract start and end of exposure. 
                % Extract movie from SPE block
                for k=1:numFrames
                    disp(k)
                    frames(:,k) = fread(fd,height,dataType)';
                 %   frameTimes(:,k) = fread(fd,2,'double')'; %%NEED TO WRITE THIS... 
                    fseek(fd, 24, 'cof'); %%skip the frame metadata!
                end
